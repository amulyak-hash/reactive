...
import { Stars, Grid, Html } from '@react-three/drei';
...
function CameraSetup() {
  ...
  useEffect(() => {
    ...
    camera.lookAt(0, 0, 0);
    // Scale zoom to viewport — balance between too big and too small
    camera.zoom = Math.min(size.width, size.height) / 18;
    camera.updateProjectionMatrix();
  }, [camera, size]);
  return null;
}

export default function IsometricScene({ useCases, onNodeClick }) {
  return (
    <Canvas
      orthographic
      dpr={[1, 1.5]}
    ...
    >
      ...
      <IsometricGraph useCases={useCases} onNodeClick={onNodeClick} />

      {/* Background — outside graph */}
